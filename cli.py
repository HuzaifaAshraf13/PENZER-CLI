...
def main():
    ...
    type_effect("[ Starting MCP Transport Node ]", 0.02, color="\033[91m")
    server_thread = threading.Thread(target=start_server, daemon=True)
    server_thread.start()
    type_effect(">> Channel bound to local machine. Inner pathways unlocked.\n", 0.02, color="\033[91m")

    agent = Agent()
    type_effect("Speak your intent.\n", 0.02, color="\033[91m")

    while True:
        user_input = input("\033[97mUser> \033[0m")  # white input
        if user_input.lower() in ["exit", "quit"]:
            type_effect("\n[ Shutting Down ] Penzer slips back into the dark mesh.", 0.02, color="\033[91m")
            break
        # Agent responses in red
